...
const assertions = [
  ...
  [/id=["']bookingForm["']/.test(html) && /id=["']flexibleDates["']/.test(html) && /id=["']adults["']/.test(html) && /id=["']children["']/.test(html) && /id=["']infants["']/.test(html) && /id=["']pets["']/.test(html), "El cotizador recopila fechas flexibles, composición del grupo y mascotas"],
  [/(Chef privado|Abastecimiento previo|Transportación|Masajes o bienestar)/.test(html) && /selectedServices/.test(siteJs), "El cotizador permite seleccionar varios servicios"],
  [/SOLICITUD DE COTIZACIÓN · VILLA CALYPSO/.test(siteJs) && /Ocupación total/.test(siteJs) && /Servicios a cotizar/.test(siteJs), "WhatsApp recibe la solicitud de cotización estructurada"],
  [/counts\.adults \+ counts\.children \+ counts\.infants > 15/.test(siteJs), "El cotizador controla la capacidad máxima de 15 huéspedes"],
  [((html.match(/class=["'][^"']*js-quote[^"']*["'][^>]*href=["']#estancia["']/g) || []).length >= 5), "Todas las acciones de cotización llevan primero al formulario"],
  [!/js-quote[^>]+wa\.me|wa\.me[^>]+js-quote/.test(html), "Ninguna acción de cotización abre un mensaje directo"],
  [!/id=["']siteIntro["']/.test(html) && /Cotizar mis fechas/.test(html), "La portada abre directamente con la acción de cotizar"],
